...
class TestRecurringTaskConfiguration(TaskApiTestCaseBase):
    ...
    def test_create_task_requires_mandatory_fields(self):
        self.reset_firestore_state()
        payload = {
        ...
        self.assertIn('task_name', response.get_json().get('error', ''))

    def test_create_task_rejects_non_dict_recurrence(self):
        self.reset_firestore_state()
        payload = {
            'task_name': 'Invalid Recurrence',
            'start_date': '2025-01-01',
            'priority_level': 2,
            'task_status': 'Ongoing',
            'owner': 'user-1',
        ...
        response = self.client.post('/api/tasks', json=payload)
        self.assertEqual(response.status_code, 400)
        self.assertIn('Invalid recurrence payload', response.get_json().get('error', ''))
    ...
    def test_completed_weekly_task_generates_next_occurrence(self):
        self.reset_firestore_state()
        base_task = self.fake_firestore._data['Tasks']['task-review']
        base_task['start_date'] = datetime(2024, 1, 1, 9, 0)  # Monday
        base_task['end_date'] = datetime(2024, 1, 2, 18, 0)
        base_task['recurrence'] = {
            'enabled': True,
            'frequency': 'weekly',
            'interval': 1,
        ...
        self.assertEqual(response.status_code, 200)
        next_instance = response.get_json().get('next_instance')
        self.assertIsNotNone(next_instance)
        self.assertTrue(next_instance['start_date'].startswith('2024-01-03T09:00:00'))
        self.assertTrue(next_instance['end_date'].startswith('2024-01-04T18:00:00'))

    def test_completed_monthly_task_generates_next_occurrence(self):
        self.reset_firestore_state()
        base_task = self.fake_firestore._data['Tasks']['task-review']
        base_task['start_date'] = datetime(2024, 1, 31, 9, 0)
        base_task['end_date'] = datetime(2024, 1, 31, 18, 0)
        base_task['recurrence'] = {
            'enabled': True,
            'frequency': 'monthly',
            'interval': 1,
        ...
        next_instance = response.get_json().get('next_instance')
        self.assertIsNotNone(next_instance)
        self.assertTrue(next_instance['start_date'].startswith('2024-02-29'))

    def test_completed_custom_weeks_task_generates_next_occurrence(self):
        self.reset_firestore_state()
        base_task = self.fake_firestore._data['Tasks']['task-review']
        base_task['start_date'] = datetime(2024, 3, 1, 9, 0)
        base_task['end_date'] = datetime(2024, 3, 1, 18, 0)
        base_task['recurrence'] = {
            'enabled': True,
            'frequency': 'custom',
            'interval': 2,
        ...
        next_instance = response.get_json().get('next_instance')
        self.assertIsNotNone(next_instance)
        self.assertTrue(next_instance['start_date'].startswith('2024-03-15'))
    ...
    def test_completed_task_respects_end_on_date_condition(self):
        self.reset_firestore_state()
        base_task = self.fake_firestore._data['Tasks']['task-review']
        base_task['start_date'] = datetime(2024, 4, 25, 9, 0)
        base_task['recurrence'] = {
            'enabled': True,
            'frequency': 'daily',
            'interval': 1,
